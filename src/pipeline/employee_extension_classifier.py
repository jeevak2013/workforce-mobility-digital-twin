...
def load_datasets():
    ...
    logger.info("Loading employee extension datasets...")

    train_df = pd.read_csv(DATA_DIR / "employee_extension_train.csv")

    test_df = pd.read_csv(DATA_DIR / "employee_extension_test.csv")
...
def prepare_features(
    ...
    X_train = train_encoded.drop(columns=[TARGET_COLUMN])

    y_train = train_encoded[TARGET_COLUMN]

    X_test = test_encoded.drop(columns=[TARGET_COLUMN])
...
def run_time_series_cv(
    ...
    for train_idx, val_idx in tscv.split(X_train):
        ...
        logger.info(f"Fold {fold} | AUC={fold_auc:.4f} | Recall={fold_recall:.4f}")
...
def export_visualizations(
    ...
    importance_df.to_csv(
        DATA_DIR / "classifier_feature_importance.csv",
        index=False,
    )
    ...
    plt.title("Employee Extension Feature Importance")

    plt.tight_layout()

    plt.savefig(DATA_DIR / "classifier_feature_importance.png")
...
def export_outputs(
    ...
    logger.info("Exporting classifier artifacts...")

    prediction_df = pd.DataFrame(
        {
            "prediction_probability": probabilities,
            "predicted_extension": predictions,
        }
    )

    prediction_df.to_csv(
        DATA_DIR / "employee_extension_predictions.csv",
        index=False,
    )
    ...
    model.save_model(str(DATA_DIR / "employee_extension_model.json"))

    logger.info("Classifier artifacts exported.")
...
def main():
    ...
    logger.info("Starting employee extension classifier...")
    ...
    logger.info("Employee extension classifier completed.")

    print("\n🚀 Employee extension classification pipeline completed.\n")
