...
HIGH_FATIGUE_EXTENSION = "EXTEND_BEYOND_0630"

MODERATE_FATIGUE_EXTENSION = "EXTEND_TO_0430"
...
def compute_churn_probability(
    ...
    probability = BASE_MONTHLY_CHURN_RATE

    # -----------------------------------------------------
    # Distance fatigue
    # -----------------------------------------------------

    if employee.home_distance_km > HIGH_DISTANCE_THRESHOLD:
        probability += 0.08

    elif employee.home_distance_km > MODERATE_DISTANCE_THRESHOLD:
        probability += 0.04
    ...
    if employee.extension_category == HIGH_FATIGUE_EXTENSION:
        probability += 0.10

    elif employee.extension_category == MODERATE_FATIGUE_EXTENSION:
        probability += 0.05
    ...
    if employee.transport_eligibility == "HUB_DROP_ONLY":
        probability += 0.07

    elif employee.transport_eligibility == "CONDITIONAL_APPROVAL":
        probability += 0.03
    ...
    if employee.gender == "Female" and employee.transport_shift in ["03:30", "04:30"]:
        probability += 0.03
    ...
    if employee.predicted_extension_minutes >= 90:
        probability += 0.05
...
def reconstruct_employee(
    ...
    shift_logout = row["shift_logout"]

    if isinstance(
        shift_logout,
        str,
    ):
        shift_logout = pd.to_datetime(shift_logout)
...
def transport_distribution_summary(
    ...
    return cast(
        Dict[str, int],
        (df["transport_shift"].value_counts().to_dict()),
    )
...
def simulate_employee_churn(
    ...
    if missing:
        raise ValueError(f"employees.csv missing columns: {missing}")
    ...
    employees: List[Employee] = [
        reconstruct_employee(row) for _, row in (employees_df.iterrows())
    ]
    ...
    for employee in employees:
        churn_probability = compute_churn_probability(employee)

        # -------------------------------------------------
        # Dynamic scaling
        # -------------------------------------------------

        churn_probability *= monthly_churn_rate / BASE_MONTHLY_CHURN_RATE
        ...
        is_churned = rng.random() < churn_probability

        if is_churned:
            churned.append(employee.model_dump())

        else:
            retained.append(employee.model_dump())
    ...
    churned_df = pd.DataFrame(churned)
    ...
    for df in [
        ...
        df["home_lat"] = df["home"].apply(lambda x: x["latitude"])

        df["home_lon"] = df["home"].apply(lambda x: x["longitude"])
    ...
    print("ENTERPRISE CHURN SIMULATION SUMMARY")

    print("=================================================\n")

    print(f"Original workforce: {len(employees_df)}")

    print(f"Employees churned: {len(churned_df)}")

    print(f"Remaining workforce: {len(retained_df)}")

    # -----------------------------------------------------
    # Transport analytics
    # -----------------------------------------------------

    remaining_transport = retained_df[retained_df["uses_company_transport"] == True]

    print(f"\nTransport users remaining: {len(remaining_transport)}")

    print("\nTransport shift distribution:")

    print(transport_distribution_summary(remaining_transport))

    print("\nExtension distribution:")

    print(retained_df["extension_category"].value_counts())

    print("\nTransport eligibility:")

    print(retained_df["transport_eligibility"].value_counts())
...
def simulate_churn_and_export(
    ...
    churned_df = simulate_employee_churn(
    ...
    print(f"Exported churn-adjusted workforce:")
...
if __name__ == "__main__":
    simulate_churn_and_export(
        ...
        output_csv=("data/employees_after_churn.csv"),
