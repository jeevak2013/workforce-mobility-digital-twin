...
def generate_attendance_status(
    ...
    for status, probability in ABSENTEEISM_CONFIG.items():
...
def generate_operational_event(
    ...
    for event, probability in OPERATIONAL_EVENT_CONFIG.items():
        cumulative += probability
...
def compute_actual_logout(
    ...
    return scheduled_logout + timedelta(minutes=extension_minutes)
...
def transport_required(
    ...
    return actual_logout.hour < 6 or (
        actual_logout.hour == 6 and actual_logout.minute <= 30
    )
...
def generate_activity_logs(
    ...
    for day_offset in range(days):
        ...
        workforce_factor = workforce_factor_for_day(current_date)
        ...
        operational_event = generate_operational_event(rng)
        ...
        for _, row in daily_df.iterrows():
            attendance_status = generate_attendance_status(rng)
            ...
            scheduled_logout = pd.to_datetime(row["shift_logout"])

            extension_minutes = int(row["predicted_extension_minutes"])
            ...
            actual_extension = max(
                extension_minutes + variability,
                0,
            )

            actual_logout = compute_actual_logout(
                scheduled_logout,
                actual_extension,
            )
            ...
            dispatch_wave = compute_dispatch_wave(
                transport_shift=row["transport_shift"],
                extension_category=row["extension_category"],
                uses_company_transport=bool(row["uses_company_transport"]),
            )
            ...
            dispatch_required = transport_required(
                actual_logout,
                bool(row["uses_company_transport"]),
            )
            ...
            risk_level = route_risk_level(
                home_distance_km=float(row["home_distance_km"]),
                requires_security_escort=bool(row["requires_security_escort"]),
            ...
            records.append(
                {
                    ...
                    "login_shift": row["login_shift"],
                    "attendance_status": (attendance_status),
                    "transport_shift": row["transport_shift"],
                    "scheduled_logout": (scheduled_logout),
                    "actual_logout": (actual_logout),
                    "extension_category": row["extension_category"],
                    "overtime_minutes": (actual_extension),
                    "actual_dispatch_wave": (dispatch_wave),
                    "uses_company_transport": bool(row["uses_company_transport"]),
                    "dispatch_required": (dispatch_required),
                    "transport_eligibility": row["transport_eligibility"],
                    "requires_security_escort": bool(row["requires_security_escort"]),
                    "operational_event": (operational_event),
    ...
    print("ENTERPRISE ACTIVITY LOG GENERATION")

    print("=================================================\n")

    print(f"Generated records: {len(logs_df)}")

    print("\nDispatch wave distribution:")

    print("\nLogin shift distribution:")

    print(logs_df["login_shift"].value_counts())

    print(logs_df["actual_dispatch_wave"].value_counts())

    print("\nOperational events:")

    print(logs_df["operational_event"].value_counts())

    print("\nRoute risk distribution:")

    print(logs_df["route_risk_level"].value_counts())
...
if __name__ == "__main__":
    generate_activity_logs(
        ...
        output_csv=("data/activity_logs.csv"),
