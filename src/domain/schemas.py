...
def to_local_projected_xy(
    ...
    r_meters = EARTH_RADIUS_KM * 1000.0

    lat_diff_rad = math.radians(lat - CENTRAL_LATITUDE)

    lon_diff_rad = math.radians(lon - CENTRAL_LONGITUDE)

    center_lat_rad = math.radians(CENTRAL_LATITUDE)

    x_meters = r_meters * lon_diff_rad * math.cos(center_lat_rad) + FALSE_EASTING

    y_meters = r_meters * lat_diff_rad
...
HUB_COORDS: Dict[
    ...
    "Thudiyalur": (11.0850, 76.9840),
    "Saravanampatti": (11.0730, 77.0600),
    "Singanallur": (11.0010, 77.0450),
    "Hopes": (10.9970, 77.0100),
    "Ganapathy": (11.0370, 77.0030),
}
...
class Coordinate(BaseModel):
    ...
    def validate_latitude(
        ...
        if not -90.0 <= value <= 90.0:
            raise ValueError("Latitude must remain between -90 and 90.")
    ...
    def validate_longitude(
        ...
        if not -180.0 <= value <= 180.0:
            raise ValueError("Longitude must remain between -180 and 180.")
...
class Hub(BaseModel):
    ...
    def validate_hub_within_radius(
        ...
        dist = self.location.distance_to_bpo()

        if dist > BPO.max_radius_km:
            raise ValueError(
                f"Hub '{self.name}' violates enterprise operational radius."
            )
...
class Employee(BaseModel):
    ...
    def effective_logout_time(
        ...
        return self.shift_logout + timedelta(minutes=self.predicted_extension_minutes)
    ...
    def requires_home_drop(
        ...
        return self.transport_eligibility in [
            "FULL_HOME_DROP",
            "CONDITIONAL_APPROVAL",
        ]
    ...
    def is_transport_user(
        ...
        return 1 if self.uses_company_transport else 0
    ...
    def clustering_feature_vector(
        ...
        logout_minutes = (
            self.effective_logout_time.hour * 60 + self.effective_logout_time.minute
        )
        ...
        return [
            ...
            # Overtime
            float(self.predicted_extension_minutes),
            # Safety
            float(self.safety_priority_score),
            float(self.female_safety_flag),
            # Transport
            float(self.is_transport_user),
        ]
...
class EmployeePool(BaseModel):
    ...
    def validate_gender_ratio(
        ...
        female_ratio = female_count / len(self.employees)

        lower = BPO.gender_female_target - BPO.gender_tolerance

        upper = BPO.gender_female_target + BPO.gender_tolerance
    ...
    def from_dataframe(
        ...
        return cls(employees=employees)
    ...
    def by_hub(
        ...
        return [emp for emp in self.employees if emp.hub == hub]

    def female_employees(
        self,
    ) -> List[Employee]:

        return [emp for emp in self.employees if emp.gender == "Female"]

    def escort_required_employees(
        self,
    ) -> List[Employee]:

        return [emp for emp in self.employees if emp.requires_security_escort]
    ...
    def average_extension_minutes(
        ...
        return sum(emp.predicted_extension_minutes for emp in self.employees) / len(
            self.employees
        )
    ...
    def total_high_priority_employees(
        ...
        return sum(
            1 for emp in self.employees if (emp.safety_priority_score >= threshold)
        )
